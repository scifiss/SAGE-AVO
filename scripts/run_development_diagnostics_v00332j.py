#!/usr/bin/env python3
"""Prepare and run the v00332j explicit structural-attention diagnostic."""
...
CONTRACT_PATH = REPOSITORY / "configs" / "development_diagnostics_v00332j.yaml"
...
def resolve() -> tuple[dict[str, Any], dict[str, Any], Path, Path, Path]:
    ...
    config["capabilities"]["explicit_structural_attention_prior"] = {
        "implemented": True,
        "enabled": True,
...
def _validate_source(contract: dict[str, Any], source: Path) -> dict[str, Any]:
    ...
    for path, field in (
        ...
        if not path.exists() or file_sha256(path) != expected[field]:
            raise RuntimeError(f"Frozen v00332i source mismatch: {path}")
    ...
    for field, count_field, hash_field in (
        ...
        if len(values) != int(expected[count_field]) or canonical_hash(values) != expected[
            ...
            raise RuntimeError(f"Frozen v00332i {field} mismatch")
    state = torch.load(state_path, map_location="cpu", weights_only=True)
    if state_sha256(state) != expected["expected_initial_model_state_sha256"]:
        raise RuntimeError("Frozen v00332i initial model-state hash mismatch")
    return {
...
def _prepared_initialization(
    ...
    expected_missing = ["graph.structural_attention_raw_strengths"]
    if compatibility.missing_keys != expected_missing or compatibility.unexpected_keys:
        raise RuntimeError(
            "v00332j initialization differs from v00332i beyond the declared strengths: "
            f"missing={compatibility.missing_keys}, unexpected={compatibility.unexpected_keys}"
        )
    state = dict(model.state_dict())
    common_equal = all(torch.equal(state[name], value) for name, value in source.items())
    if not common_equal:
        raise RuntimeError("A common v00332i model tensor changed during v00332j initialization")
    strengths = F.softplus(state["graph.structural_attention_raw_strengths"])
    metadata = {
        ...
        "initial_positive_strengths": strengths.tolist(),
...
def prepare(_: argparse.Namespace) -> None:
    ...
    if any(path.exists() for path in outputs):
        raise FileExistsError("v00332j contract already exists; refusing to replace frozen inputs")
    ...
    initialization["selection_and_probes_byte_identical"] = (
        file_sha256(source_selection) == file_sha256(selection_path)
        and file_sha256(source_probes) == file_sha256(probes_path)
    )
...
def train(args: argparse.Namespace) -> None:
    ...
    if not all(path.exists() for path in _paths(experiment)):
        raise FileNotFoundError("Run the prepare command before v00332j training")
    prepared = json.loads(contract_path.read_text(encoding="utf-8"))
    if prepared["contract_sha256"] != canonical_hash(contract):
        raise RuntimeError("v00332j contract is stale; create a new revision")
    if prepared["resolved_config_sha256"] != canonical_hash(config):
        raise RuntimeError("v00332j resolved config is stale; create a new revision")
    ...
    if initialization["prepared_state_file_sha256"] != file_sha256(state_path):
        raise RuntimeError("v00332j initial state file SHA-256 mismatch")
    selection = json.loads(selection_path.read_text(encoding="utf-8"))
    run = experiment / "runs" / "full"
    ...
    for epoch in range(completed + 1, target + 1):
        ...
        print(f"Completed v00332j epoch {epoch} and its checkpoint-only probes")
