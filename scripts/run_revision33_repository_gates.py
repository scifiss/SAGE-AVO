#!/usr/bin/env python3
"""Run and record the bounded Revision-3.3 repository release gates."""
    ...
    for path in sorted((REPOSITORY / "notebooks").glob("0[1-5]_*.ipynb")):
        ...
        for index, cell in enumerate(notebook.cells):
            ...
            try:
                ...
                parse_errors.append({"cell": index, "error": str(error)})
        records.append(
            {
                "notebook": path.name,
...
def main() -> None:
    ...
    destination = (
        ...
        / "revision33"
        / "v0033_validation8_dry_frame_supported"
        / "reports"
    ...
    report = {
        ...
        "status": "passed" if all(row["passed"] for row in commands) and notebook["passed"] else "failed",
    }
