#!/usr/bin/env python3
"""Execute private Revision-3.3 validation copies of notebooks 01--05."""
...
def _locations() -> tuple[Path, Path]:
    ...
    validation = private / "revision33" / VALIDATION_NAME
...
def _patch_private_copy(
    ...
    notebook.metadata["revision33_validation"] = {
    ...
    try:
        for path in sorted((REPOSITORY / "notebooks").glob("0[1-5]_*.ipynb")):
            ...
            output = destination / path.name.replace(
                ".ipynb", ".v0033_validation.executed.ipynb"
            )
