#!/usr/bin/env python3
"""Run and persist the Revision-3.3.2a numerical-correction repository gates."""
...
OUTPUT = PRIVATE / "revision332a" / "masked_physics_correction" / "repository_gates.json"
COMMANDS = (
    ...
    (
        "training_observability_tests",
        ["pytest", "-q", "-p", "no:cacheprovider", "tests/test_training_observability.py"],
    ),
    (
        "masked_physics_reference_tests",
        ["pytest", "-q", "-p", "no:cacheprovider", "tests/test_masked_physics_loss.py"],
    ),
)
