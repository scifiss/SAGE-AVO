#!/usr/bin/env python3
"""Prepare and run the v00332k task-decoupled graph diagnostic."""
...
CONTRACT_PATH = REPOSITORY / "configs" / "development_diagnostics_v00332k.yaml"
...
def resolve() -> tuple[dict[str, Any], dict[str, Any], Path, Path, Path]:
    ...
    strengths = graph["structural_attention_prior"]["initial_strengths"]
    ...
    config["model"]["experimental_graph"] = {
        ...
        "structural_prior_initial_strengths": [
            [float(value) for value in strengths["tangential"]],
            [float(value) for value in strengths["normal"]],
        ],
    }
    config["capabilities"]["task_decoupled_graph_routing"] = {
        "implemented": True,
        "enabled": True,
...
def _validate_source(
    ...
    for path, field in (
        ...
        if not path.exists() or file_sha256(path) != expected[field]:
            raise RuntimeError(f"Frozen v00332j source mismatch: {path}")
    ...
    for field, count_field, hash_field in (
        ...
        if (
            ...
            raise RuntimeError(f"Frozen v00332j {field} mismatch")
    state = torch.load(state_path, map_location="cpu", weights_only=True)
    if state_sha256(state) != expected["expected_initial_model_state_sha256"]:
        raise RuntimeError("Frozen v00332j initial model-state hash mismatch")
    ...
    if parameter_count != int(expected["expected_architecture_parameter_count"]):
        raise RuntimeError("v00332k unexpectedly changed trainable parameter capacity")
    if state_sha256(model.state_dict()) != expected["expected_initial_model_state_sha256"]:
        raise RuntimeError("v00332k did not preserve every v00332j model tensor")
    return {
        ...
        "initial_model_state_sha256": state_sha256(state),
        "all_model_tensors_byte_identical": True,
...
def prepare(_: argparse.Namespace) -> None:
    ...
    if any(path.exists() for path in outputs):
        raise FileExistsError("v00332k contract already exists; refusing to replace frozen inputs")
    ...
    verification["selection_probe_and_state_files_byte_identical"] = all(
        file_sha256(source_path) == file_sha256(target_path)
        for source_path, target_path in (
            (source_selection, selection_path),
            (source_probes, probes_path),
            (source_state, state_path),
        )
    )
    initialization_path.write_text(json.dumps(verification, indent=2) + "\n", encoding="utf-8")
    payload = {
        ...
        "initial_model_state_sha256": verification["initial_model_state_sha256"],
    }
...
def train(args: argparse.Namespace) -> None:
    ...
    if not all(path.exists() for path in _paths(experiment)):
        raise FileNotFoundError("Run the prepare command before v00332k training")
    prepared = json.loads(contract_path.read_text(encoding="utf-8"))
    if prepared["contract_sha256"] != canonical_hash(contract):
        raise RuntimeError("v00332k contract is stale; create a new revision")
    if prepared["resolved_config_sha256"] != canonical_hash(config):
        raise RuntimeError("v00332k resolved config is stale; create a new revision")
    ...
    if initialization["initial_state_file_sha256"] != file_sha256(state_path):
        raise RuntimeError("v00332k initial state file SHA-256 mismatch")
    selection = json.loads(selection_path.read_text(encoding="utf-8"))
    run = experiment / "runs" / "full"
    ...
    for epoch in range(completed + 1, target + 1):
        ...
        print(f"Completed v00332k epoch {epoch} and its checkpoint-only probes")
