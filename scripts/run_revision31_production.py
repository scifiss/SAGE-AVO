#!/usr/bin/env python3
"""Run guarded Revision-3.1 production stages using only versioned v0031 paths.
...
def _contracts() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Path]]:
    ...
    synthetic = load_config(REPOSITORY / "configs" / "synthetic_s01_v0031.yaml")
    dataset = load_config(REPOSITORY / "configs" / "ml_dataset_s01_v0031.yaml")
    training = load_config(REPOSITORY / "configs" / "sage_avo_s01_v0031.yaml")
    private = Path(paths["private_artifact_root"])
    source_snapshot = load_frozen_source_reference(
        private, pointer_name="v0031_production_current.json"
    )
...
def train(args: argparse.Namespace) -> None:
    ...
    if last.exists() and not args.resume:
        raise FileExistsError(
            f"Existing v0031 run found at {run_directory}; use --resume to continue it"
        )
