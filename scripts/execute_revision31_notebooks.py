#!/usr/bin/env python3
"""Execute local Revision-3.1 validation copies of notebooks 01--05.

Notebook 01 is redirected to an isolated clone of the Stage-01 tree so the
canonical v003 artifacts are not rewritten. Notebooks 02--05
consume the bounded Revision-3.1 root through the existing validation-root
contract. Repository notebooks are not modified by this script.
"""
...
def _locations() -> tuple[Path, Path]:
    ...
    validation = private / "revision31" / VALIDATION_NAME
...
def _prepare_notebook01_clone(work_data_root: Path, validation_root: Path) -> Path:
    ...
    source = work_data_root / "s01data"
    destination = clone_root / "s01data"
...
def _patch_private_copy(
    ...
    if name.startswith(("04_", "05_")):
        ...
        for cell in notebook.cells:
            ...
            raise RuntimeError(f"Could not make the validation experiment path generic in {name}")
    notebook.metadata["revision31_validation"] = {
...
def main() -> None:
    ...
    try:
        for path in sorted((REPOSITORY / "notebooks").glob("0[1-5]_*.ipynb")):
            ...
            output = destination / path.name.replace(".ipynb", ".v0031_validation.executed.ipynb")
    ...
    report = {
        "status": "complete" if all(row["error_outputs"] == 0 for row in records) else "failed",
